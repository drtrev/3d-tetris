...
class Texture {
  ...
  private:
    static std::vector <char*> filenames;
    struct Image {
      unsigned sizeX;
      unsigned sizeY;
      char *data;
  ...
  public:
    static std::vector <int> texId; // the texture numbers
    ...
    static int imageLoad(char *filename, Image *image)
    {
      ...
      if ((file = fopen(filename, "rb"))==NULL)
      {
        ...
        std::cerr << "Set TEXTURE_PATH environment variable?" << std::endl;
        return 0;
      }
    ...
    static void loadTextures()
    {
      ...
      for(unsigned int x = 0; x < filenames.size(); x++)
      {
        ...
        {
          std::cerr << "Error allocating space for texture" << std::endl;
          std::cerr << "Texture: " << x << " -- " << filenames[x] << std::endl;
          exit(0);
        }
