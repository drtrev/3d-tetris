...
class Block : public Pawn {
  private:
    ...
    std::vector <std::vector <int> > newPosition; // store new position of blocks
    std::vector <std::vector <int> > lastStored; // store old position/collision trail
    ...
    void drawWallMark(std::vector <int>&);
    bool originalCheckCollision(std::vector <Block>&, float[]);
  ...
  public:
    ...
    float getTargetY();
    float getTargetZ();
    void move(std::vector <Block> &blocks, float[],
    ...
    bool removeLayer(std::vector <Block>, std::vector <Block>&, int[GAMEAREA_WIDTH][GAMEAREA_HEIGHT][GAMEAREA_DEPTH], float[], int&, int);
    std::vector <std::vector <float> > getLayer(int);
    void hit();
    void unGrounded();
