...
std::vector <int> layerFound;
std::vector <int> timeLayerFound;
...
void newBlock()
{
  ...
  if (LOG_OUTPUT) std::cerr << "newBlockCount: " << newBlockCount << std::endl;
  newBlockTimer = clock() + (long int) (newBlockCount * CLOCKS_PER_SEC);
  //std::cout << "new block info ready to send: " << type << std::endl;
}

// Output traffic for analysis
void analyseTraffic()
{
  if (LOG_OUTPUT) std::cout << traffic << std::endl;
...
void receiveData()
{
  ...
  for (int i = 0; i < clCount; i++) {
    ...
    if (FD_ISSET(clSocks[i], &readSocks)) {
      ...
      if (numbytes == 0){
        close(clSocks[i]);       
        std::cerr << "a connection was closed" << std::endl;
        ...
        // store
        //buf[numbytes] = '\0'; // make like a std::string
        //printf("received buffer: %s\n", buf);
        //std::cout << "id assigning: " << clSocks[i] << " which goes to char: " << (char) clSocks[i] << std::endl;
        ...
        for (int j = 0; j < numbytes; j++) {
          ...
          if (receivedCursor[i] > MAXRECVDATASIZE - 2) { // -2 because 2 can be added at once from above
            std::cerr << "warning: permanent store dataReceived[" << i << "] was overflowed: reset took place" << std::endl;
            receivedCursor[i] = 0;
            numbytes = 0;
            //overflow = true;
          }
  ...
  for (int i = 0; i < clCount; i++) {
    if (clSocks[i] == -1) {
      std::cerr << "removed connection from clSocks[]" << std::endl;
...
void sendData()
{
  ...
  for (int i = 0; i < MAXCLIENTS; i++) {
    if (dataReceived[i][0] != '\0') { // if there's data
      ...
      for (int j = 0; j < receivedCursor[i]; j++) {
        if (dataReceived[i][j] == 2) {
          ...
          for (int k = j+1; k < receivedCursor[i]; k++) {
            if (k == 0) std::cerr << "error" << std::endl;
      ...
      if (startOfData > -1 && endOfData > -1) {
        //std::cout << "about to send: start: " << startOfData << " end: " << endOfData << " rc: " << receivedCursor[i] << std::endl;
        // random check
        if (endOfData <= startOfData) {
          std::cerr << "hold on, startOfData: " << startOfData << ", endOfData: " << endOfData
               << ", I thought I fixed this by initialising vars each loop" << std::endl;
          // nuke it
        ...
        if (dataToSend[2] == FLAG_LAYER_FOUND) {
          ...
          std::cerr << "received layer found: " << layerFound[layerFound.size()-1] << std::endl;
          ...
          if (timeLayerFound.size() > 0) {
            if (secondsElapsed() - timeLayerFound[0] > LAYER_FOUND_EXPIRY_TIME) {
              ...
              layerFound.pop_back();
              timeLayerFound.pop_back();
              std::cerr << "layer found message expired" << std::endl;
            }
          }
          ...
          for (int j = 0; j < clCount; j++) {
            ...
            if (i != j) {
              //std::cout << "sending to socket: " << j << std::endl;
              //std::cout << "sending data: " << dataReceived[i] << std::endl;
  ...
  if (serverMessage[0] > 0) {
    if (LOG_OUTPUT) std::cerr << "sending a message to " << clCount << " clients. messNum: " << messNum++ << std::endl;
  ...
  if (serverMessage[0] > 0) {
    if (LOG_OUTPUT) std::cerr << "message cleared" << std::endl;
...
int main(int argc, char** argv)
{
  ...
  while (1) { // keep listening and serving
    ...
    if (FD_ISSET(listenSock, &readSocks)) {
      //std::cout << "listen sock readable!" << std::endl;
      serverAccept();
      numReadable--;
