...
// an eight character std::string
//   num - the number to make an eight character std::string
//
// Returns:
//   an eight character std::string equivalent of num
std::string Client::dealZeros(float num)
{ // makes num an 8 character std::string
  std::string result = "";
...
//   humans - the std::vector storing all humans in the system
//
// Returns:
//   the element number containing the new human
int Client::makeNewHuman(int id, std::vector <Human>& humans)
{
...
//   networkData - a std::string of data to send
void Client::sendData(std::string networkData)
{
...
void Client::transmitBufferedData()
{
  ...
  if (numReadable > 0) {
    ...
    //std::cout << "sending: ";
    //for (int i = 0; i < amountOfDataRead; i++) std::cout << buffer[i];
    //std::cout << std::endl;
...
//   blocks - a std::vector storing all the game blocks
...
void Client::manipulateBlock(std::vector <Block> &blocks, int blockNum, char axis, int direction)
{
  if (blockNum < 0 || blockNum > (int) blocks.size() - 1) {
    std::cerr << "Client::manipulateBlock(): blockNum out of range: " << blockNum << std::endl;
  }else{
...
//   blocks - the std::vector storing all the game blocks
//   blockNum - the number of the block to be moved
//   direction - the direction in which to move the game block
void Client::moveBlock(std::vector <Block> &blocks, int blockNum, int direction)
{
  if (blockNum < 0 || blockNum > (int) blocks.size() - 1) {
    std::cerr << "Client::moveBlock(): blockNum out of range: " << blockNum << std::endl;
  }else{
    ...
    switch (direction) {
      ...
      case 5:
        ...
        break;
      default:
        std::cerr << "Client::moveBlock(): direction not recognised: " << direction << std::endl;
    }
...
void Client::doClient(Pawn& player, std::vector <Human>& humans, std::vector <Block> &blocks, int &blockType, bool &gravity, bool &receivedLock, std::vector <int> &receivedRemoveLayer)
{
  // client stuff
  ...
  if (numReadable > 0) { // there's data to read
    ...
    if (numbytes > MAXRECVDATASIZE - 1)
      std::cout << "warning: " << numbytes << " received in one go - bigger than MAXRECVDATASIZE!" << std::endl;

    //std::cout << "received: ";
    //for (int i = 0; i < numbytes; i++) std::cout << buf[i];
    //std::cout << std::endl;
    ...
    //buf[numbytes] = '\0'; // for outputting to screen, make like a std::string
    ...
    if (id == -1) {
      if (buf[0] == 'i') { // this should be first
        ...
        std::cout << "I have been assigned an id of: " << id << std::endl;
        if (buf[startIndex++] == FLAG_MASTER) {
          master = true;
          std::cout << "I am the master (primary client)!" << std::endl;
        }else{
          master = false;
          std::cout << "I am the slave (secondary client)!" << std::endl;
        }
       //std::cout << "full buffer: " << buf << std::endl;
      }else{
        std::cout << "still waiting for id" << std::endl;
      }
    }
    ...
    if (id > -1) { // we've got our id (i.e. not still waiting for it)
      ...
      for (int i = startIndex; i < numbytes; i++) {
        ...
        if (receivedCursor > MAXRECVDATASIZE - 1) {
          std::cout << "permanent store receivedSoFar was overflowed: reset took place" << std::endl;
          receivedCursor = 0;
          numbytes = 0;
          overflow = true;
        }
      ...
      char flag = FLAG_NONE;
      bool negative = false;
      std::vector <float> data; // a temporary store of the data
      ...
      for (int i = 0; i < receivedCursor; i++) {
        if (receivedSoFar[i] == 2) {
          startIndex = i+1;
          originalIndex = i;
          if (overflow) std::cout << "overflow recovered" << std::endl;
      ...
      if (receivedCursor > startIndex + 1) { // NOTE this is repeated below
        //std::cout << "in block thing" << std::endl;
      ...
      while ((flag == FLAG_POSITION && chunk > 39) || (flag == FLAG_LOCK && chunk > 0) || (flag == FLAG_MANIPULATE && chunk > 2)
        ...
        if (id != dataId) {
          ...
          if (flag == FLAG_POSITION && humanId > -1) {
            ...
            //std::cout << "setting stuff: x: " << data[0] << " y: " << data[1] << " z: " << data[2] << std::endl;
            ...
            //  std::cerr << "client::doClient: lockNumber out of range: " << lockNumber << std::endl;
            //}else{
            ...
            //std::cout << "received new block" << std::endl;
            ...
            std::cout << "received remove layer data unit: " << receivedRemoveLayer[(int) receivedRemoveLayer.size()-1] << std::endl;
          }else if (flag == FLAG_LAYER_FOUND) {
            std::cout << "received flag layer found... shouldn't have" << std::endl;
            startIndex++;
            // it's for the server only
          }else{
            std::cout << "flag was not found: " << flag << std::endl;
            std::cout << "in int format: " << (int) flag << std::endl;
          }
        } // end if it's not you
        else{
          std::cout << "WARNING: data is not for me - I should not have been sent this" << std::endl;
          ...
          switch (flag) {
            ...
            case FLAG_MOVE:
              ...
              break;
            case FLAG_NEW_BLOCK:
              std::cerr << "error: got a server message for new block but dataId was same as me" << std::endl;
              startIndex ++;
              break;
            case FLAG_BLOCK:
              startIndex += 19 * 8 + 1;
              break;
            case FLAG_GRAVITY:
              std::cerr << "error: got a server message for gravity but dataId was same as me" << std::endl;
              break;
        ...
        if (receivedCursor > startIndex + 2) { // TODO remove std::couts here
          //std::cout << "more data" << std::endl;
          if (receivedSoFar[startIndex] == 2) startIndex++; // there should be a STX here
          dataId = (int) receivedSoFar[startIndex++];
          flag = receivedSoFar[startIndex++];
          //if (flag == FLAG_LOCK) std::cout << "got a flag lock!!" << std::endl;
  ...
  if (readyToSend) {
    ...
    std::string networkData = FLAG_POSITION + dealZeros(player.getX()) + dealZeros(player.getY()) + dealZeros(player.getZ())
    ...
    if (sendBlock < (int) blocks.size()) {
      if (!blocks[sendBlock].getMoving() && !blocks[sendBlock].getTurning() && master) { // && blocks[sendBlock].getGrounded()) 
        ...
        // but because blocks may be split (and the std::vector change) the receiver needs to identify blocks
        ...
        //std::cout << "sending confirmedY: " << blocks[sendBlock].getY() << ", as dealZeros: " << dealZeros(blocks[sendBlock].getY()) << std::endl;
  ...
  if (moveBuf.getDataOnBuffer() > 1) {
    ...
    if (moveBuf.getChar(tempBlockNum, 0)) {
      ...
      if (tempBlockNumInt > (int) blocks.size() -1 || tempBlockNumInt < 0) {
        std::cerr << "client::doClient(): tempBlockNumInt out of range for moving blocks: " << tempBlockNumInt << std::endl;
  ...
  if (manipBuf.getDataOnBuffer() > 2) {
    ...
    if (manipBuf.getChar(tempBlockNum, 0)) {
      ...
      if (tempBlockNumInt > (int) blocks.size() -1 || tempBlockNumInt < 0) {
        std::cerr << "client::doClient(): tempBlockNumInt out of range for manipulating blocks: " << tempBlockNumInt << std::endl;
      }else if (!blocks[tempBlockNumInt].getMoving() && !blocks[tempBlockNumInt].getTurning()) {
        char c[3];
