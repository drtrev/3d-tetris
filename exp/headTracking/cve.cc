...
// a std::vector to store other players in the game
...
// a std::vector to store the blocks in the game
std::vector <Block> blocks;
int selectedBlock = -1; // id of block selected
int selectedX = 0, selectedY = 0;

// the collision array
//std::vector <std::vector <std::vector <int> > > collisionArray;
...
std::vector <int> sentFoundLayer;
std::vector <int> receivedRemoveLayer;
...
float fps;
std::vector <int> fpsStore; // store FPS values
...
std::vector <int> Texture::texId;
std::vector <char*> Texture::filenames;
// this is needed so filenames can be kept and referenced by other functions in texture.h

// Tracker
const char *cfgname = "peripheral_example.cfg";
float tracker_pos[IRI_TRACKER_MAX_SENSOR * 3];
float tracker_quat[IRI_TRACKER_MAX_SENSOR * 4];
float tracker_pos_last[3];
iriTracker *tracker = new iriTracker( cfgname, "tracker0" );
//float headTrackSensitivity = 20.0;
float headTrackSensitivity = 30.0;
...
void drawObject( const ShadowedObject& object )
{
  ...
  //glDisable(GL_CULL_FACE);
	...
	for ( int i = 0; i < object.nFaces; i++ )
	{
      ...
      //std::cout << "j: " << j << "vx: " << vertex.x << " vy: " << vertex.y << " vz: " << vertex.z << std::endl;
		}
	}
	glEnd();
...
void makeShadowBlocks()
{
  ...
  for (int i = 0; i < 8; i++) {
        ...
        for (int x = 0; x < 5; x++) {
          if (b.getData(x, y, z) == 1) {
            ...
            if (x == 0 || b.getData(x-1, y, z) == 0) {
              ...
              //std::cout << "face: " << face << ", allocatedVertex: " << allocatedVertex << ", vertex: " << vertex << std::endl;
    ...
    //std::cout << "nFaces: " << shadowBlock[i].nFaces << std::endl;
    //std::cout << "nVertices: " << shadowBlock[i].nVertices << std::endl;
  }
}
...
// Clears and resets the three-dimensional collision array (std::vector)
...
void init()
{
  ...
  // initialise tracker vars
  tracker_pos_last[0] = 0.0;
  tracker_pos_last[1] = 0.0;
  tracker_pos_last[2] = 0.0;
...
//
// str - the std::string of text to be displayed
// x, y - the coordinates for the text
void displayText(std::string str, float x, float y)
{
  float initX = x;
  glPushMatrix();
	glLoadIdentity();
...
void drawLocalConnection()
{
  ...
  if (selectedBlock > -1) {
    blocks[selectedBlock].getPivotCoords(x1, y1, z1);
      ...
      // rotate std::vector about Y axis
      ...
      // add a negative z std::vector to make it so when you look down you don't see a gap in the line
      float x4 = 0.0, z4 = -1.0;
...
void drawRemoteConnection(int blockNum, int humanNum)
{
  ...
  if (humanNum < 0 || humanNum > (int) humans.size() - 1){
    std::cerr << "drawConnection: humanNum out of range: " << humanNum << std::endl;
  }else if (blockNum < 0 || blockNum > (int) blocks.size() -1) {
    // we can output things here for testing purposes
    // 
    //std::cerr << "drawConnection: blockNum out of range: " << blockNum << std::endl;
    //std::cerr << "...assuming it's just because it's not appeared yet or because blocks have split" << std::endl;
...
void display()
{
  ...
  if (renderMode == GL_RENDER){
    ...
    if (stereo) {
      glPushMatrix();
      glScalef(0.2, 1.0, 0.2); // draw smaller ground so it fits within display
    }
    drawGround();
    ...
    //if (humans.size() > 1) std::cout << "in cve.cc humans.size() detected as: " << humans.size() << std::endl;
 ...
 // if (renderMode == GL_RENDER) {
    ...
    /*cout << "x " << tracker_pos[0] << " y " << tracker_pos[1]
      << " z " << tracker_pos[2]
      << " qx " << tracker_quat[IRI_QX] << " qy " << tracker_quat[IRI_QY]
      << " qz " << tracker_quat[IRI_QZ] << " qw " << tracker_quat[IRI_QW]
      << endl;
    */
    /*std::string str2 = "x ";
    str2 += tracker_pos[0];
    ...
    str2 += tracker_pos[2];
    str = str2.c_str();*/
    sprintf(str, "x %f y %f z %f", tracker_pos[0], tracker_pos[1], tracker_pos[2]);
    displayText(str, windowWidth - 300, -windowHeight + 100);
...
void reshape(int w, int h)
{
  ...
  windowCentreX = w / 2, windowCentreY = h / 2;
}
  ...
  switch (key) {
    case 27:  // esc
      ...
      if (network && !training && LOG_OUTPUT) {
        std::cout << "closing network connection - return value: " << client.closeConnection() << std::endl;
      }
      // output fps values
      /*for (int i = 0; i < (int) fpsStore.size(); i++) {
        std::cout << fpsStore[i] << std::endl;
    ...
    case 'o':
      ...
      blocks[0].output();
      std::cout << "lc0: " << lightCoords0[0] << ", " << lightCoords0[1] << ", " << lightCoords0[2] << std::endl;
      std::cout << "lc1: " << lightCoords1[0] << ", " << lightCoords1[1] << ", " << lightCoords1[2] << std::endl;
      break;
    case 'p':
...
void processHits(GLint hits, GLuint buffer[])
{
   ...
   if( nearestId > -1) {
     ...
     if (nearestId > (int) blocks.size()) {
       std::cerr << "selectedId too high in process hits" << std::endl;
...
void unGroundAbove(float y)
{
  for (int i = 0; i < (int) blocks.size(); i++) {
    ...
    //else std::cout << "Checking y: " << y << " against block y: " << blocks[i].getY() << std::endl;
  }
}
...
{
  std::string networkData; // data to be sent across network
  ...
  if (pressLeftButton && selectedBlock > -1) {
    ...
    if (!blocks[selectedBlock].getTurning() && !blocks[selectedBlock].getMoving()) {
      ...
      if (pAngleY == 90 || pAngleY == 270) {
        if (abs(mx) > abs(my) + 1) { // turn right or left
          ...
          if (angleY > 359.0 || angleY < 0.1 || angleY > 179.0 && angleY < 181.0) {
            ...
            std::cout << "turning in X" << std::endl;
      ...
      if (networkData.length() == 4) { // axis part will only be put in if we actually turned the block
        ...
        //std::cout << "(network dependent) sent manipulate info: " << networkData << std::endl;
      }
    } // end if not turning
    ...
    if (!blocks[selectedBlock].getMoving() && !blocks[selectedBlock].getTurning()) {
      ...
      if ((abs(mx) - abs(my)) > 1 || (abs(my) - abs(mx)) > 1) {
        ...
        if (network && !training) {
          ...
          networkData += temp;
          client.sendData(networkData);
          //std::cout << "(network dependent) sent move info: " << networkData << std::endl;
        }
      }
    }
...
void gravity(int value)
{
  ...
  if (pauseGame) std::cout << "number grounded: " << grounded << std::endl;
  ...
  for (int layerY = 0; layerY <= (int) BLOCK_START_Y; layerY += 5) {
    ...
    if ((int) layer.size() == (GAMEBLOCK_WIDTH * GAMEBLOCK_DEPTH)) {
      ...
      if (network && !training) {
        ...
        // has been found
        if (!found) {
          if (LOG_OUTPUT) std::cout << "sending found flag" << std::endl;
      ...
      if (!network || training || found) {
        ...
        if (LOG_OUTPUT) std::cout << "layer removed at time: " << seconds << " since start of this game." << std::endl;
        outputBlocks.clear();
...
void newGame()
{
  ...
  // output stats
  if (LOG_OUTPUT) {
    std::cout << "Final score for game number " << gameNumber << ": " << score << std::endl;
    std::cout << "Block score: " << blockScore << std::endl;
    std::cout << "Number of layers: " << numberOfLayers << std::endl;
    std::cout << "Time that game lasted: " << ((glutGet(GLUT_ELAPSED_TIME) - gameStartTime) / 1000.0) << " seconds" << std::endl;
    std::cout << "Distance moved by player (master == " << client.getMaster() << "): " << player.getDistanceMoved() << std::endl;
    std::cout << "Number of cubes left: " << numberOfCubesLeft << std::endl;
    std::cout << "Gravity timer reached: " << gravityTimer << std::endl;
    std::cout << "New block timer: " << newBlockTimer << std::endl;
  }

  gameNumber++;
  if (LOG_OUTPUT) std::cout << std::endl << "Beginning game " << gameNumber << std::endl;
...
void idle(int value)
{
  ...
  //
  // Instruct tracker to get new pos/orient data. The data is stored
  // internally by the tracker class.
  //
  tracker->getNewPosQuat();
  //
  // As above, and return that data in supplied parameters
  //
  tracker->getNewPosQuat( tracker_pos, tracker_quat );
  //
  // Obtain pos/orient data that was stored by call to getNewPosQuat()
  //
  tracker->getCurrPosQuat( tracker_pos, tracker_quat );


  float headRight = -tracker_pos[0] + tracker_pos_last[0];
  float headForwards = -tracker_pos_last[1] + tracker_pos[1];
  float headUp = -tracker_pos_last[2] + tracker_pos[2];
  player.moveRight(headRight * headTrackSensitivity);
  player.moveForwards(headForwards * headTrackSensitivity);
  player.changeGround(headUp * headTrackSensitivity, SECS_PER_FRAME);
  tracker_pos_last[0] = tracker_pos[0];
  tracker_pos_last[1] = tracker_pos[1];
  tracker_pos_last[2] = tracker_pos[2];
  ...
  if (network && !training) {
    ...
    if (startGravity) {
      if (LOG_OUTPUT) std::cout << "starting gravity" << std::endl;
      glutTimerFunc(gravityTimer, gravity, 0);
    }
    if (!client.getMaster()) {
      me.setTexFace(2);
      for (int i = 0; i < (int) blocks.size(); i++) {
        if (blocks[i].getMoving() || blocks[i].getTurning()) {
          ...
          //std::cout << "moving/turning" << std::endl;
        }
...
int main(int argc, char** argv)
{
  ...
  //if (argc > 1) {
    ...
    for (int i = 1; i < argc; i++) {
      ...
      if (!strcmp(argv[i], "-h") || !strcmp(argv[i], "--help")) {
        ...
        std::cout << "  ./<programName> [options] | [<ipAddress>]" << std::endl;
        std::cout << "  where 'ipAddress' is the address of the computer running the server." << std::endl;
        std::cout << std::endl;
        std::cout << "Programs:" << std::endl;
        std::cout << "  ./cve (four rounds single player then collaborative)" << std::endl;
        std::cout << "  ./multiuser (straight into collaborative mode)" << std::endl;
        std::cout << "  ./server (run the server program)" << std::endl;
        std::cout << std::endl;
        std::cout << "Options:" << std::endl;
        std::cout << "  -s  Enable stereoscopic mode" << std::endl;
        std::cout << "  -l  Enable true shadows" << std::endl;
        std::cout << "  -h, --help" << std::endl;
        std::cout << "      Display this info" << std::endl;
        exit(0);
      }
    }
    if (stereo) std::cout << "Stereoscopic." << std::endl;
    else std::cout << "No stereo." << std::endl;
    if (shadow) std::cout << "Shadows." << std::endl;
    else std::cout << "No shadows." << std::endl;
    if (gotIpAddress)
      std::cout << "Will be using ip address: " << argv[ipArg] << " for collaborative games." << std::endl;
    else std::cout << "Standalone." << std::endl;
  /*}else{
    gotIpAddress = false;
    std::cout << "Standalone." << std::endl;
    std::cout << "No stereo." << std::endl;
  }*/
  ...
  //const char *cfgname = "peripheral_example.cfg";
  //tracker_pos[IRI_TRACKER_MAX_SENSOR * 3];
  //tracker_quat[IRI_TRACKER_MAX_SENSOR * 4];
  //*tracker = new iriTracker( cfgname, "tracker0" );
