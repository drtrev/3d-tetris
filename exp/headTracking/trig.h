...
class Trig {
  ...
  public:
    ...
    static float sn(float n) {
      ...
      if (n > 360) n -= 360;
      if (n > 360 || n < -360) {
        std::cerr << "Trig::sn() - n out of range. n = " << n << std::endl;
        return 0;
      }
    ...
    static float cs(float n) {
      if (n > 360) n -= 360;
      if (n > 360 || n < -360) {
        std::cerr << "Trig::cs() - n out of range. n = " << n << std::endl;
        return 0;
      }
    ...
    static float asn(float n) {
      ...
      if ((int) n < 0 || (int) n > 120) {
        std::cout << "Trig::asn() - n out of range. n = " << n << " - original: " << (n/40.0-1.0) << std::endl;
      }
