      ...
      for (int x = 0; x < 5; x++) {
        if (data[x][y][z] == 1) { // if there's a block here
          ...
          if (worldX < 0 || worldY < 0 || worldZ < 0 ||
            ...
            std::cerr << "Block::Block - attempt to check a world coordinate outside of collision array bounds" << std::endl;
            std::cerr << "worldX: " << worldX << ", worldY: " << worldY << ", worldZ: " << worldZ << std::endl;
          }else{
            if (collisionArray[worldX][worldY][worldZ] > 0) {
              //std::cout << "Block constructor detected GAME OVER" << std::endl;
              gameOver = true;
            }
...
void Block::setType(int t)
{
  ...
  std::vector <int> temp; // temporary store to initialise 2D std::vector
  std::vector <std::vector <int> > temp2; // temporary store to initialise 3D std::vector

  data.clear();

  // make 5x5x5 3D std::vector
  temp.push_back(0);
  temp.push_back(0);
  temp.push_back(0);
  temp.push_back(0);
...
void Block::turnToTarget()
{
  ...
  if (target1 && target2 && target3) {
    turning = false;
    matrixCopy(matrix, oldMatrix);
    std::cout << "before loop: x,y,z: " << angleX << ", " << angleY << ", " << angleZ << std::endl;
    loopAngles();
    targetAngleX = angleX, targetAngleY = angleY, targetAngleZ = angleZ;
    std::cout << "after loop: x,y,z: " << angleX << ", " << angleY << ", " << angleZ << std::endl;
  }
}
...
void Block::move(std::vector <Block> &blocks, float boundaries[],
...
{
  targetAngleX = a;
  //std::cout << std::endl << "x " << targetAngleX << std::endl;
  turning = true;
}
...
{
  targetAngleY = a;
  //std::cout << "y " << targetAngleY << std::endl;
  turning = true;
}
...
{
  targetAngleZ = a;
  //std::cout << "z " << targetAngleZ << std::endl;
  turning = true;
}
...
void Block::output()
{
  std::cout << "Turned X: " << turnedX << ", turnedY: " << turnedY << ", turnedZ: " << turnedZ << std::endl;
  std::cout << "Angle X: " << angleX << ", angleY: " << angleY << ", angleZ: " << angleZ << std::endl;
  ...
  float wx = 0, wy = 0, wz = 0;
  worldCoords(wx, wy, wz);
  std::cout << "World coords of (0, 0, 0): " << roundf(wx) << ", " << roundf(wy) << ", " << roundf(wz) << std::endl;
  wx = 1, wy = 0, wz = 0;
  worldCoords(wx, wy, wz);
  std::cout << "World coords of (1, 0, 0): " << roundf(wx) << ", " << roundf(wy) << ", " << roundf(wz) << std::endl;
}
...
void Block::drawWallMark(std::vector <int> &texId)
{
  glPushMatrix();
...
void Block::draw(std::vector <int>& texId, bool selected)
{
  glPushMatrix();
  //glLoadIdentity();
  ...
  for( l1=0; l1<3; l1++ ) { // Columns
    ...
    for( l2=0; l2<3; l2++ ) { // Rotational elements in column
      ...
      if( fabsf( m1[element] ) < ROUNDING ) {
        ...
        std::cout << "Block::matrixCorrectRoundingError " << m1[element] << std::endl;
      }
    }
  }
}
...
{
  if (i < 0 || i > 15) {
    std::cerr << "Block::setMatrix: out of range: " << i << std::endl;
  }else
    matrix[i] = v;
}
...
{
  if (i < 0 || i > 15) {
    std::cerr << "Block::setConfirmedMatrix: out of range: " << i << std::endl;
  }else
    confirmedMatrix[i] = v;
}
...
{
  if (i < 0 || i > 15) {
    std::cerr << "Block::getConfirmedMatrix: out of range: " << i << std::endl;
    return 0.0;
  }else
...
void Block::toConfirmed(float boundaries[], int collisionArray[GAMEAREA_WIDTH][GAMEAREA_HEIGHT][GAMEAREA_DEPTH])
{
  ...
  //if ( ( (y < confirmedY - 10.0 || y > confirmedY + 5.0) && (confirmedX != oldConfirmedX || confirmedY != oldConfirmedY || confirmedZ != oldConfirmedZ)) || grounded) {
    //if (!grounded) std::cout << "y: " << y << ", confirmedY: " << confirmedY << std::endl;
  if (turning || moving) std::cerr << "Block::toConfirmed: called when block active!!" << std::endl;
  if (confirmedX != oldConfirmedX || confirmedY != oldConfirmedY || confirmedZ != oldConfirmedZ || grounded) {
    //if (id == 27) std::cout << "setting to Confirmed: " << id << std::endl;
    ...
    if (safelyStoredSize != (int) lastStored.size()) {
      std::cerr << "Block::toConfirmed(): lastStored.size() != safelyStoredSize" << std::endl;
      std::cerr << "lastStored.size() == " << lastStored.size() << ", safelyStoredSize == " << safelyStoredSize << std::endl;
    }

    gotConfirmed = false;

    //if (matrix[0] > 0.9 && matrix[0] < 1.1) {
    /*  std::cout << "matrix for " << id;
      for (int i = 0; i < 16; i++) {
        if (i%4 == 0) std::cout << std::endl;
        std::cout << matrix[i];
      }
...
//   blocks - std::vector storing all the game blocks
//   boundaries - the game area boundaries
//
// Returns:
//   true if hit, false otherwise
bool Block::originalCheckCollision(std::vector <Block> &blocks, float boundaries[])
{
...
bool Block::checkCollision(int collisionArray[GAMEAREA_WIDTH][GAMEAREA_HEIGHT][GAMEAREA_DEPTH], float boundaries[])
{
  ...
  std::vector <int> pos; // temporary position std::vector
  newPosition.clear();
      ...
      for (int x = 0; x < 5; x++) {
        if (data[x][y][z] == 1 && !hit) { // if there's a block here
          ...
          for (int i = 0; i < 5; i++) {
            ...
            //std::cout << "[0]: " << collisionArray[0].size() << ", wy: " << worldY << std::endl;
            //std::cout << "wx: " << wx << ", wy: " << wy << ", wz: " << wz << std::endl;
            //std::cout << "worldX: " << worldX << ", worldY: " << worldY << ", worldZ: " << worldZ << std::endl;

            if (worldX < 0 || worldY < 0 || worldZ < 0 ||
                worldX >= GAMEAREA_WIDTH || worldY >= GAMEAREA_HEIGHT ||
                worldZ >= GAMEAREA_DEPTH) {
              //std::cerr << "Block::checkCollision - attempt to check a world coordinate outside of collision array bounds" << std::endl;
              //std::cerr << "worldX: " << worldX << ", worldY: " << worldY << ", worldZ: " << worldZ << std::endl;
  ...
  if (!hit) { // we're clear to move to new position
    ...
    // new position now contains only the new position std::vectors
  }else{
...
void Block::clearCollisionTrail(int collisionArray[GAMEAREA_WIDTH][GAMEAREA_HEIGHT][GAMEAREA_DEPTH], bool storeNewPosition)
{
  ...
  if (storeNewPosition) {
    ...
    if ((int) lastStored.size() != safelyStoredSize) {
      std::cerr << "Block::clearCollisionTrail: lastStored.size() != safelyStoredSize" << std::endl;
      std::cerr << "lastStored.size() == " << lastStored.size() << ", safelyStoredSize == " << safelyStoredSize << std::endl;
    } else {
      for (int i = 0; i < (int) lastStored.size(); i++)
...
//   false otherwise
bool Block::removeLayer(std::vector <Block> blocks, std::vector <Block> &outputBlocks, int collisionArray[GAMEAREA_WIDTH][GAMEAREA_HEIGHT][GAMEAREA_DEPTH], float boundaries[], int &blockId, int layerY)
{
        ...
        if (data[x][y][z] == 1) { // if there's a block here
          ...
          if (wy > layerY - 0.1 && wy < layerY + 0.1) { // does it match layerY
            ...
            if (worldX < 0 || worldY < 0 || worldZ < 0 ||
                ...
                worldX >= GAMEAREA_WIDTH || worldY >= GAMEAREA_HEIGHT ||
                worldZ >= GAMEAREA_DEPTH) {
              std::cerr << "Block::removeLayer - attempt to check a world coordinate outside of collision array bounds" << std::endl;
              std::cerr << "worldX: " << worldX << ", worldY: " << worldY << ", worldZ: " << worldZ << std::endl;
            }else
      ...
      for (int y = 0; y < 5; y++) {
        for (int x = 0; x < 5; x++) {
          if (data[x][y][z] == 1) { // if there is a block here
            ...
            //std::cout << "about to add a block" << std::endl;
            // add the new split blocks to the output std::vector
...
// Return std::vectors containing x and z positions at a particular layer
//   layerY - the y coordinate of the layer
std::vector <std::vector <float> > Block::getLayer(int layerY)
{
  std::vector <float> temp;
  std::vector <std::vector <float> > layer; // all the position std::vectors
...
void Block::hit()
{
  //std::cout << "oldX,Y,Z: " << oldX << ", " << oldY << ", " << oldZ << std::endl;
