...
char ClientWrapper::intToChar(int n) {
  char c;
  switch (n)
  {
    case 0:
...
int ClientWrapper::charToInt(char c)
{
  int v = -1;
...
string ClientWrapper::dealZeros(float num)
...
int ClientWrapper::makeNewHuman(int id, vector <Human>& humans)
{
...
void ClientWrapper::sendData(string networkData)
{
...
void ClientWrapper::transmitBufferedData()
{
...
void ClientWrapper::manipulateBlock(vector <Block> &blocks, int blockNum, char axis, int direction)
{
  if (blockNum < 0 || blockNum > (int) blocks.size() - 1) {
    cerr << "ClientWrapper::manipulateBlock(): blockNum out of range: " << blockNum << endl;
  }else{
...
void ClientWrapper::moveBlock(vector <Block> &blocks, int blockNum, int direction)
{
  if (blockNum < 0 || blockNum > (int) blocks.size() - 1) {
    cerr << "ClientWrapper::moveBlock(): blockNum out of range: " << blockNum << endl;
  }else{
    ...
    switch (direction) {
      ...
      case 5:
        ...
        break;
      default:
        cerr << "ClientWrapper::moveBlock(): direction not recognised: " << direction << endl;
    }
...
void ClientWrapper::doClientWrapper(Pawn& player, vector <Human>& humans, vector <Block> &blocks, int &blockType, bool &gravity, bool &receivedLock, vector <int> &receivedRemoveLayer)
{
  // client stuff
      ...
      while (((flag == FLAG_POSITION && chunk > 39) || (flag == FLAG_LOCK && chunk > 0) || (flag == FLAG_MANIPULATE && chunk > 2)
        ...
        if (id != dataId) {
          ...
          if (flag == FLAG_POSITION && humanId > -1) {
            ...
            //  cerr << "client::doClientWrapper: lockNumber out of range: " << lockNumber << endl;
            //}else{
  ...
  if (moveBuf.getDataOnBuffer() > 1) {
    ...
    if (moveBuf.getChar(tempBlockNum, 0)) {
      ...
      if (tempBlockNumInt > (int) blocks.size() -1 || tempBlockNumInt < 0) {
        cerr << "client::doClientWrapper(): tempBlockNumInt out of range for moving blocks: " << tempBlockNumInt << endl;
  ...
  if (manipBuf.getDataOnBuffer() > 2) {
    ...
    if (manipBuf.getChar(tempBlockNum, 0)) {
      ...
      if (tempBlockNumInt > (int) blocks.size() -1 || tempBlockNumInt < 0) {
        cerr << "client::doClientWrapper(): tempBlockNumInt out of range for manipulating blocks: " << tempBlockNumInt << endl;
      }else if (!blocks[tempBlockNumInt].getMoving() && !blocks[tempBlockNumInt].getTurning()) {
        char c[3];
...
void ClientWrapper::setHost(const char* ip)
{
...
void ClientWrapper::setReadyToSend(int value)
{
  readyToSend = value;
}
...
int ClientWrapper::closeConnection()
{
  return close(sockfd);
}
